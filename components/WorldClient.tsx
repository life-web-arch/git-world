...
function FullScreenLoader() { /* ... unchanged ... */ }
const fetcher = (url: string) => fetch(url).then(r => r.json());
function PlayerTracker({ username, room }: { username: string, room: any }) { /* ... unchanged ... */ }
function OtherPlayer({ position, rotation, username }: any) { /* ... unchanged ... */ }
function DevBuilding({ dev, position }: { dev: any, position: [number, number, number] }) { /* ... unchanged ... */ }
function City() { /* ... unchanged ... */ }
...
export default function WorldClient({ username }: { username: string }) {
  ...
  return (
    // THE FIX: Wrap the entire component in the EcctrlProvider
    <EcctrlProvider>
...
// Re-pasting helper components to ensure they exist in this script block
...
PlayerTracker = function({ username, room }) { const ref = useRef(); let lastSend = 0; useFrame(({ clock }) => { if (clock.elapsedTime - lastSend > 0.1 && ref.current && room) { const pos = ref.current.getWorldPosition(new THREE.Vector3()); const rot = ref.current.getWorldQuaternion(new THREE.Quaternion()); room.send({ type: 'broadcast', event: 'move', payload: { username, position: [pos.x, pos.y, pos.z], rotation:[rot.x, rot.y, rot.z, rot.w] } }); lastSend = clock.elapsedTime; } }); return <group ref={ref} />; }
OtherPlayer = function({ position, rotation, username }) { const ref = useRef(); useFrame(() => { if (ref.current) { ref.current.position.lerp(new THREE.Vector3(...position), 0.1); ref.current.quaternion.slerp(new THREE.Quaternion(...rotation), 0.1); } }); return ( <group ref={ref}><mesh position={[0, 1, 0]}><capsuleGeometry args={[0.5, 1, 4]} /><meshStandardMaterial color="#3b82f6" /></mesh><Text position={[0, 2.8, 0]} fontSize={0.6} outlineWidth={0.05} outlineColor="black">{username}</Text></group> ); }
DevBuilding = function({ dev, position }) { const height = Math.max(5, (dev.contributions || 10) / 100); const width = Math.max(4, Math.min(25, (dev.repos || 5) / 2)); return ( <RigidBody type="fixed" position={position}><mesh position={[0, height / 2, 0]}><boxGeometry args={[width, height, width]} /><meshStandardMaterial color={`hsl(${(dev.username.length * 25) % 360}, 70%, 20%)`} emissive={`hsl(${(dev.username.length * 25) % 360}, 70%, 40%)`} emissiveIntensity={0.5} /></mesh>{dev.avatar_url && <Image url={dev.avatar_url} transparent scale={[width * 0.8, width * 0.8, 1]} position={[0, height/2, width/2 + 0.1]} />}{/* We use avatar_url from supabase now */}<Text position={[0, height + 2, 0]} fontSize={2} outlineWidth={0.08} outlineColor="#000000">{dev.username}</Text></RigidBody> ); }
City = function() { const { data: devs } = useSWR('/api/city'); if (!devs || !Array.isArray(devs)) return null; return ( <group>{devs.map((dev, i) => { const cols = Math.max(2, Math.ceil(Math.sqrt(devs.length))); const spacing = 40; const x = (i % cols) * spacing - (cols * spacing) / 2; const z = Math.floor(i / cols) * spacing - (cols * spacing) / 2; return <DevBuilding key={dev.username || i} dev={dev} position={[x, 0, z]} />; })}</group> ); }
